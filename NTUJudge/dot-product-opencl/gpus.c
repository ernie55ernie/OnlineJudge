...
int main(int argc, char *argv[]) {
    ...
    while (scanf("%d %" PRIu32 " %" PRIu32, &N, &key1, &key2) == 3) {
        ...
        for(i = 0; i < GPU_id_got; i++){
            ...
            status = clSetKernelArg(kernels[i], 3, sizeof(unsigned int), (void*)&N);

            //size_t globalworkoffset = (size_t)offset * sizeof(cl_uint);
            size_t localworksize = number;
            size_t globalworksize = (n_per_device[i] / number + (n_per_device[i] % number != 0)) * number;
